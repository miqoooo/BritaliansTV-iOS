...
extension ItemModel: Identifiable {
    ...
    var videoRow: [RowModel] {
        return self.CK_video_details_row?.map({
            RowModel(title: $0.title, items: $0.row_items.compactMap({ ItemModel(rowItemModel: $0)}))
        }) ?? []
    }
}
...
extension ItemModel {
    init(rowItemModel: RowItemsModel) {
        ...
        self.CK_id = String(rowItemModel.details_name.utf8)
